...
func (s *NeoService) IndexHistoryOfDisorder() error {
	...
	result, err := session.ReadTransaction(func(tx neo4j.Transaction) (interface{}, error) {
		...
		result, err := tx.Run("MATCH (n:ObjectConcept {sctid: '312850006', active: '1'})<-[*1..3]-(children)-[:HAS_DESCRIPTION]->(description: Description {descriptionType: 'Synonym'}) RETURN description", nil)
		if err != nil {
			return nil, err
		}
	...
	docs, total, err := c.Search(redisearch.NewQuery("History of *diverticulitis*").
		Limit(0, 20).
		SetReturnFields("term"))

...
const SocialHistoryForm: React.FC<Props> = ({
  updateId,
  encounter,
  onSuccess,
}) => {
    ...
    if (updateId) {
      ...
      getCondition(updateId)
        .then((res) => {
          ...
          if (code) {
            setSelectedSocialHistory({ value: code.code, label: code.display });
          }
  ...
  const searchSocialHistory = useCallback(
  ...
  const onSubmit = async (input: any) => {
    ...
    try {
      ...
      const condition: Condition = {
        ...
        code: selectedSocialHistory
          ? {
              coding: [
                {
                  code: selectedSocialHistory.value,
                  display: selectedSocialHistory.label,
                  system: "http://snomed.info/sct",
                },
              ],
              text: selectedSocialHistory.label,
            }
          : undefined,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="mb-10">
      <p className="text-lg font-bold tracking-wide text-teal-700 uppercase">
        {updateId ? "Update Social History" : "Add Social History"}
      </p>

      <CodedInput
        title="Social History"
        conceptId="160476009"
        selectedItem={selectedSocialHistory}
        setSelectedItem={setSelectedSocialHistory}
        searchOptions={searchSocialHistory}
      />

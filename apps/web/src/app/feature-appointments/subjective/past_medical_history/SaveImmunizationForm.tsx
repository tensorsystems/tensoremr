...
const SEARCH_IMMUNIZATION = gql`
  query SearchImmunization($size: Int!, $searchTerm: String!) {
    immunizationConcepts(size: $size, searchTerm: $searchTerm) {
      term
      sctid
    }
  }
...
export const SaveImmunizationForm: React.FC<Props> = ({
  ...
  const immunizationQuery = useLazyQuery<Query, QueryImmunizationConceptsArgs>(
    SEARCH_IMMUNIZATION
  );
  ...
  const loadOptions = (
    ...
    if (inputValue.length > 0) {
      immunizationQuery[0]({
        ...
        const values = resp.data?.immunizationConcepts.map((e) => ({
          value: e?.sctid,
          label: e?.term,
        }));

...
import {
  ...
  ConceptAttributes,
  MutationSavePatientClinicalFindingHistoryArgs,
  Query,
  QueryConceptAttributesArgs,
  QueryHistoryClinicalFindingConceptsArgs,
...
const SEARCH_HISTORY_CLINICAL_FINDING_CONCEPTS = gql`
  query HistoryClinicalFindingConcepts($size: Int!, $searchTerm: String!) {
    historyClinicalFindingConcepts(size: $size, searchTerm: $searchTerm) {
      term
      sctid
    }
  }
...
const SAVE_CLINICAL_FINDING_HISTORY = gql`
  mutation SaveClinicalFindingHistory($input: ClinicalFindingInput!) {
    savePatientClinicalFindingHistory(input: $input) {
      id
    }
  }
`;
...
export const SaveClinicalFindingHistoryForm: React.FC<Props> = ({
  patientChartId,
  onSuccess,
  onCancel,
}) => {
  ...
  const clinicalFindingQuery = useLazyQuery<
    Query,
    QueryHistoryClinicalFindingConceptsArgs
  >(SEARCH_HISTORY_CLINICAL_FINDING_CONCEPTS);
  ...
  const loadOptions = (
    ...
    if (inputValue.length > 0) {
      clinicalFindingQuery[0]({
        variables: {
          size: 100,
          searchTerm: inputValue,
        },
      }).then((resp) => {
        const values = resp.data?.historyClinicalFindingConcepts.map((e) => ({
          value: e?.sctid,
          label: e?.term,
        }));
  ...
  const save = useMutation<any, MutationSavePatientClinicalFindingHistoryArgs>(
    SAVE_CLINICAL_FINDING_HISTORY,
    {
      onCompleted(data) {

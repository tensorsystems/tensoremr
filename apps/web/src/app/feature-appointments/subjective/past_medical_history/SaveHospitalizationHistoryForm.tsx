...
import {
  ...
  ConceptAttributes,
  MutationSavePatientHospitalizationHistoryArgs,
  Query,
  QueryConceptAttributesArgs,
  QueryHospitalAdmissionConceptsArgs,
...
const SEARCH_HOSPITAL_ADMISSION_CONCEPTS = gql`
  query HospitalAdmissionConcepts($size: Int!, $searchTerm: String!) {
    hospitalAdmissionConcepts(size: $size, searchTerm: $searchTerm) {
      term
      sctid
    }
  }
...
const SAVE_HOSPITALIZATION_HISTORY = gql`
  mutation SavePatientHospitalizationHistory($input: ClinicalFindingInput!) {
    savePatientHospitalizationHistory(input: $input) {
      id
    }
  }
`;
...
export const SaveHospitalizationHistoryForm: React.FC<Props> = ({
  patientChartId,
  onSuccess,
  onCancel,
}) => {
  ...
  const hospitalizationQuery = useLazyQuery<
    Query,
    QueryHospitalAdmissionConceptsArgs
  >(SEARCH_HOSPITAL_ADMISSION_CONCEPTS);
  ...
  const loadOptions = (
    ...
    if (inputValue.length > 0) {
      hospitalizationQuery[0]({
        variables: {
          size: 100,
          searchTerm: inputValue,
        },
      }).then((resp) => {
        const values = resp.data?.hospitalAdmissionConcepts.map((e) => ({
          value: e?.sctid,
          label: e?.term,
        }));
  ...
  const save = useMutation<any, MutationSavePatientHospitalizationHistoryArgs>(
    SAVE_HOSPITALIZATION_HISTORY,
    {
      onCompleted(data) {

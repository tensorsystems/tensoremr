...
import {
  ...
  Query,
  QueryAdministrativeStatusConceptsArgs,
  QueryConceptAttributesArgs,
} from '@tensoremr/models';
import { useForm } from 'react-hook-form';
import { ConceptBrowser } from '@tensoremr/ui-components';

const SEARCH_ADMINISTRATIVE_STATUS = gql`
  query SearchAdministrativeStatus($size: Int!, $searchTerm: String!) {
    administrativeStatusConcepts(size: $size, searchTerm: $searchTerm) {
      term
      sctid
    }
  }
...
const SAVE_ADMINISTRATIVE_STATUS = gql`
  mutation SaveAdministrativeStatus {
    id
  }
`;
...
export const SaveAdministrativeForm: React.FC<Props> = ({
  patientHistoryId,
  onSuccess,
  onCancel,
  onSaveChange,
  ...
  const administrativeStatusQuery = useLazyQuery<
    Query,
    QueryAdministrativeStatusConceptsArgs
  >(SEARCH_ADMINISTRATIVE_STATUS);
  ...
  const loadOptions = (
    ...
    if (inputValue.length > 0) {
      administrativeStatusQuery[0]({
        variables: {
          size: 100,
          searchTerm: inputValue,
        },
      }).then((resp) => {
        const values = resp.data?.administrativeStatusConcepts.map((e) => ({
          value: e?.sctid,
          label: e?.term,
        }));
      ...
      <div className="w-1/2">
        ...
        <form>
          <p className="text-2xl font-extrabold tracking-wide">
            Add Administrative History
          </p>

          <div className="mt-4">
            <div>
              <AsyncSelect
                placeholder={'Administrative history of ...'}
                cacheOptions={false}
                defaultOptions
                isClearable={true}
